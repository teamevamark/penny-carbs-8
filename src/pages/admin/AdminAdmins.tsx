...
type PermissionState = Record<PermissionKey, PermValue>;

interface AdminUser extends PermissionState {
  id: string;
  user_id: string;
  created_at: string;
  updated_at: string;
...
const permissionLabels: { key: PermissionKey; label: string }[] = [
  ...
  { key: 'perm_users', label: 'Manage Users' },
  { key: 'perm_storage', label: 'Manage Storage' },
];
...
const AdminAdmins: React.FC = () => {
  ...
  const [formData, setFormData] = useState<PermissionState & { user_id: string }>({
    user_id: '',
    ...emptyPermissions(),
  });
  ...
  const fetchAdmins = async () => {
    ...
    try {
      ...
      if (adminIds.length > 0) {
        ...
        const adminsWithProfiles = (data as AdminUser[]).map((admin) => ({
          ...admin,
  ...
  const handleOpenDialog = (admin?: AdminUser) => {
    if (admin) {
      ...
      const next: PermissionState & { user_id: string } = {
        user_id: admin.user_id,
        ...emptyPermissions(),
      };
      PERMISSION_KEYS.forEach((key) => {
        const v = (admin[key] as PermValue) ?? 'none';
        next[key] = v === 'read' || v === 'write' ? v : 'none';
      });
      setFormData(next);
    } else {
      ...
      setMobileSearch('');
      setSearchResults([]);
      setFormData({ user_id: '', ...emptyPermissions() });
    }
  ...
  const handleSaveAdmin = async () => {
    ...
    try {
      ...
      PERMISSION_KEYS.forEach((key) => {
        permissionData[key] = formData[key];
      });
      ...
      if (editingAdmin) {
        ...
        // Ensure the user has the admin role. Replace any existing role
        // (e.g., customer) since user_roles has a unique(user_id) index.
        ...
        if (deleteRoleError) {
          console.warn('Could not clean prior role:', deleteRoleError);
        }
            ...
            <div className="space-y-3">
              ...
              {permissionLabels.map(({ key, label }) => (
                <div key={key} className="flex items-center space-x-2">
                  <Checkbox
                    id={key}
                    checked={formData[key] !== 'none'}
                    onCheckedChange={(checked) =>
                      setFormData({
                        ...formData,
                        [key]: checked ? 'write' : 'none',
                      })
                    }
                  />
